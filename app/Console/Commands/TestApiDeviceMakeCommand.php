...
class TestApiDeviceMakeCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'make:test-api-device {name : The name of the class} '.
     ...
     * @var string
     */
    protected $description = 'Create a new test api device class';
     ...
     * @var string
     */
    protected $type = 'Test Api Device';
    ...
    protected function getStub()
    {
        ...
        if ($this->option('parent')) {
            return __DIR__.'/stubs/test.api.device.nested.stub';
        } elseif ($this->option('model')) {
            return __DIR__.'/stubs/test.api.device.model.stub';
        } elseif ($this->option('resource')) {
            return __DIR__.'/stubs/test.api.device.stub';
        }

        return __DIR__.'/stubs/test.api.device.plain.stub';
    }
    ...
    {
        return parent::getDefaultNamespace($rootNamespace).'\ApiDevice';
    }
    ...
    /**
     * Replace the Device model namespace.
     *
     * @param  string  $stub
     * @return string
     */
    ...
    {
        if (! config('auth.providers.devices.model')) {
            return $stub;
        }

        return str_replace(
            $this->laravel->getNamespace().'User',
            config('auth.providers.devices.model'),
            $stub
        );
    }
    ...
    protected function getRouteName($name)
    {
        ...
        $name = 'ApiDevice\\'.Str::replaceLast('ControllerTest', '', $name);
        $names = explode('\\', $name);
