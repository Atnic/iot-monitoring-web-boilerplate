 ...
 */
class ControllerApiDeviceMakeCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $name = 'make:controller-api-device';
     ...
     * @var string
     */
    protected $description = 'Create a new controller api device class';
     ...
     * @var string
     */
    protected $type = 'Controller ApiDevice';
    ...
    protected function getStub()
    {
        if ($this->option('parent')) {
            return __DIR__.'/stubs/controller.api.device.nested.stub';
        } elseif ($this->option('model')) {
            return __DIR__.'/stubs/controller.api.device.model.stub';
        } elseif ($this->option('resource')) {
            return __DIR__.'/stubs/controller.api.device.stub';
        }

        return __DIR__.'/stubs/controller.api.device.plain.stub';
    }
    ...
    {
        return $rootNamespace.'\Http\Controllers\ApiDevice';
    }
    ...
    protected function createTest()
    {
        ...
        $this->call('make:test-api-device', [
    ...
    protected function appendRouteFile()
    {
        ...
        $file = base_path('routes/api/device.php');
        $routeName = $this->getRouteName($name);
        $routePath = $this->getRoutePath($name);

        $routeDefinition = 'Route::get(\''.$routePath.'\', \'ApiDevice\\'.$nameWithoutNamespace.'\')->name(\''.$routeName.'\');'.PHP_EOL;
        ...
        if ($this->option('parent') || $this->option('model') || $this->option('resource')) {
            ...
            if (count($asExploded) > 1) {
                ...
                $routeDefinition = 'Route::apiResource(\''.$routePath.'\', \'ApiDevice\\'.$nameWithoutNamespace.'\', [ \'as\' => \'api.device.'.$as.'\' ]);'.PHP_EOL;
            } else {
                $routeDefinition = 'Route::apiResource(\''.$routePath.'\', \'ApiDevice\\'.$nameWithoutNamespace.'\', [ \'as\' => \'api.device\' ]);'.PHP_EOL;
            }
        }
    ...
    protected function getRouteName($name)
    {
        ...
        $name = 'ApiDevice\\'.Str::replaceLast('Controller', '', $name);
        $names = explode('\\', $name);
        ...
        $name = implode('.', $names);
        $name = str_replace_first('api_device', 'api.device', $name);
    ...
    {
        $routeName = str_replace_first('api.device.', '', $this->getRouteName($name));
